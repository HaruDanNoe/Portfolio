    ...
    <section id="projects" className="py-24 bg-white">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex flex-col md:flex-row justify-between items-end mb-16 gap-6">
          <div>
            <span className="font-display text-primary font-bold uppercase tracking-widest text-sm mb-4 block">Portfolio</span>
          ...
          <div className="flex gap-4">
            <button className="px-6 py-2 bg-dark text-white rounded-full text-sm font-bold">All</button>
            <button className="px-6 py-2 bg-gray-100 text-gray-500 rounded-full text-sm font-bold hover:bg-primary hover:text-dark transition-all">Web Design</button>
            <button className="px-6 py-2 bg-gray-100 text-gray-500 rounded-full text-sm font-bold hover:bg-primary hover:text-dark transition-all">UI/UX</button>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
          {PROJECTS.map((project, index) => (
            ...
            >
              ...
              {/* Alternative overlay for mobile or static view */}
              <div className="p-8 md:hidden bg-white">
                <span className="text-primary font-bold text-xs uppercase tracking-wider mb-1 block">{project.category}</span>
                <h3 className="text-xl font-display font-bold text-dark">{project.title}</h3>
              </div>
            </motion.div>
          ))}
        </div>
